...
const KomAuftragDetail: React.FC = () => {
    ...
    const ohneLieferdatum = (obj: any) => {
        if (!obj) return obj;
        const { lieferdatum, artikelPosition, createdAt, updatedAt, __v, ...rest } = obj;
        return rest;
    };
        ...
        const fetchData = async () => {
            try {
                ...
                if (auftragData.artikelPosition?.length) {
                    ...
                    // Leergut- / abgeleitete Positionen (mit positionId) ausblenden
                    const sichtbarePositionen = pos.filter(p => !p.leergutVonPositionId);
