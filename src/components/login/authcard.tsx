...
const composeAdresse = (street: string, plz: string, ort: string) => {
    const s = street.trim();
    const p = plz.trim();
    const o = ort.trim();
    if (!s && !p && !o) return '';
...
const parseAdresse = (adresse?: string): { street: string; plz: string; ort: string } => {
...
const AuthCard: React.FC<AuthCardProps> = ({
    ...
    // Lieferzeit (Register): Start/End Zeitfenster, gespeichert als String in register.values.lieferzeit
    const [lieferWindows, setLieferWindows] = useState<string[]>([]);
    const [lieferStart, setLieferStart] = useState<string>('');
    const [lieferEnde, setLieferEnde] = useState<string>('');

    // Adresse (Register): gesplittet, gespeichert als String in register.values.adresse
    const [adresseStrasse, setAdresseStrasse] = useState<string>('');
    const [adressePlz, setAdressePlz] = useState<string>('');
    const [adresseOrt, setAdresseOrt] = useState<string>('');
    ...
    const addLieferWindow = () => {
        if (!lieferStart || !lieferEnde) return;
        if (lieferEnde <= lieferStart) {
            setMessage({ type: 'danger', text: 'Endzeit muss nach der Startzeit liegen.' });
            return;
        }
        const w = `${lieferStart}–${lieferEnde}`;
        setLieferWindows((prev) => {
            const next = Array.from(new Set([...prev, w]));
            register.set({ lieferzeit: next.join(', ') });
            return next;
        });
        setLieferStart('');
        setLieferEnde('');
    ...
    const removeLieferWindow = (w: string) => {
        setLieferWindows((prev) => {
    ...
    const clearLieferWindows = () => {
        setLieferWindows([]);
        register.set({ lieferzeit: '' });
        setLieferStart('');
        setLieferEnde('');
    };
    ...
    const getError = (field: string): string | null => {
        ...
        switch (field) {
            ...
            case 'email':
                return !v.email.trim() ? 'E‑Mail ist erforderlich.' : (!validateEmail(v.email) ? 'Ungültige E-Mail-Adresse.' : null);
            case 'password':
                                ...
                                <li className="nav-item" role="presentation">
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            const current = (register.values.lieferzeit || '')
                                                .split(',')
                                                .map((x) => x.trim())
                                                .filter(Boolean);
                                            setLieferWindows(current);

                                            const a = parseAdresse(register.values.adresse);
                                            setAdresseStrasse(a.street);
                                            setAdressePlz(a.plz);
                                            setAdresseOrt(a.ort);
                                        }}
                                        type="button"
                                        ...
                                        <form onSubmit={handleRegisterSubmit} noValidate>
                                            <div className="row g-3 mb-3">
                                                ...
                                                <div className="col-md-6">
                                                    ...
                                                    <div className="input-group input-group-lg">
                                                        ...
                                                        <input
                                                            id="regEmail"
                                                            type="email"
                                                            className={`${fieldClass} ${getError('email') ? 'is-invalid' : ''}`}
                                                ...
                                                <div className="col-12">
                                                    ...
                                                    <div className="row g-2">
                                                        <div className="col-md-6">
                                                            <div className="input-group input-group-lg">
                                                                ...
                                                                <input
                                                                    id="regAdresseStrasse"
                                                                    type="text"
                                                                    className={`${fieldClass} ${getError('adresse') ? 'is-invalid' : ''}`}
                                                                    placeholder="Straße + Hausnummer"
                                                                    value={adresseStrasse}
                                                                    onChange={(e) => {
                                                                        const street = e.target.value;
                                                                        setAdresseStrasse(street);
                                                                        register.set({ adresse: composeAdresse(street, adressePlz, adresseOrt) });
                                                                    }}
                                                                    onBlur={() => setTouched((t) => ({ ...t, adresse: true }))}
                                                                    required
                                                                />
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="col-md-3">
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    setAdressePlz(plz);
                                                                    register.set({ adresse: composeAdresse(adresseStrasse, plz, adresseOrt) });
                                                                }}
                                                                onBlur={() => setTouched((t) => ({ ...t, adresse: true }))}
                                                            />
                                                        </div>
                                                        ...
                                                        <div className="col-md-3">
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    setAdresseOrt(ort);
                                                                    register.set({ adresse: composeAdresse(adresseStrasse, adressePlz, ort) });
                                                                }}
                                                                onBlur={() => setTouched((t) => ({ ...t, adresse: true }))}
                                                            />
                                                        </div>
                                                    ...
                                                    {getError('adresse') && <div className="invalid-feedback d-block">{getError('adresse')}</div>}
                                                ...
                                                <div className="col-12">
                                                    ...
                                                    <div className="input-group input-group-lg">
                                                        ...
                                                        <input
                                                            ...
                                                            className={`${fieldClass} ${getError('password') ? 'is-invalid' : ''}`}
                                                    ...
                                                    {getError('password') && <div className="invalid-feedback d-block">{getError('password')}</div>}
                                                </div>
                                                <div className="col-12">
                                                    ...
                                                    <div className="input-group input-group-lg">
                                                        ...
                                                        <input
                                                            id="regConfirm"
                                                            type="password"
                                                            className={`${fieldClass} ${getError('confirmPassword') ? 'is-invalid' : ''}`}
                                                    ...
                                                    {getError('confirmPassword') && <div className="invalid-feedback d-block">{getError('confirmPassword')}</div>}
                                                </div>
                                            </div>
                                            ...
                                            <div className="mb-3 mt-3">
                                                ...
                                                <div className="input-group input-group-lg">
                                                    ...
                                                    <input
                                                        ...
                                                        autoComplete="tel"
                                                        value={register.values.telefon}
                                                        onChange={(e) => register.set({ telefon: e.target.value })}
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="mb-3">
                                                <label className="form-label fw-semibold">Lieferzeit (Zeitfenster)</label>

                                                <div className="row g-2 align-items-end">
                                                    <div className="col-md-5">
                                                        <div className="input-group input-group-lg">
                                                            <span className="input-group-text"><Icon name="bi-clock" /></span>
                                                            <select
                                                                className={fieldClass}
                                                                value={lieferStart}
                                                                onChange={(e) => setLieferStart(e.target.value)}
                                                            >
                                                    ...
                                                    <div className="col-md-5">
                                                        <div className="input-group input-group-lg">
                                                            <span className="input-group-text"><Icon name="bi-clock-history" /></span>
                                                            <select
                                                                className={fieldClass}
                                                                value={lieferEnde}
                                                                onChange={(e) => setLieferEnde(e.target.value)}
                                                            >
                                                ...
                                                {lieferWindows.length > 0 && (
                                                    <div className="mt-2 d-flex flex-wrap gap-2">
                                                        {lieferWindows.map((w) => (
                                                            <span key={w} className="badge bg-light text-dark border">
                                                                {w}
                                                                <button
                                                                    type="button"
                                                ...
                                                {lieferWindows.length > 0 && (
                                                    <button type="button" className="btn btn-link p-0 small mt-2" onClick={clearLieferWindows}>
                                                        Auswahl löschen
                                                    </button>
                                                )}

                                                <div className="form-text">Start & Ende auswählen und mit + hinzufügen. Mehrere Zeitfenster möglich.</div>
                                            </div>
